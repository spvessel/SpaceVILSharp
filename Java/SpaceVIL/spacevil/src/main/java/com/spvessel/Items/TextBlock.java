...
public class TextBlock extends VisualItem implements InterfaceTextEditable, InterfaceDraggable, InterfaceTextShortcuts, InterfaceScrollable {
    ...
    private List<ItemAlignment> _blockAlignment = new LinkedList<>(Arrays.asList(ItemAlignment.LEFT, ItemAlignment.TOP));
    ...
    private final int UpArrowCode = 328;
    private final int DownArrowCode = 336;
    private final int EndCode = 335;
    private final int HomeCode = 327;
    private final int ACode = 30;
    private final int EnterCode = 28;
    ...
    public TextBlock() {
        ...
        ShiftValCodes = new LinkedList<>(Arrays.asList(LeftArrowCode, RightArrowCode, EndCode, HomeCode, UpArrowCode, DownArrowCode));
    ...
    protected void onScrollUp(Object sender, MouseArgs args) {
        int h = getTextHeight();
        if (h < _cursorYMax) return;
        if (globalYShift >= 0) return;
    ...
    protected void onScrollDown(Object sender, MouseArgs args) {
        int h = getTextHeight();
        if (h < _cursorYMax) return;
        if (h + globalYShift <= _cursorYMax) return;
    ...
    private void replaceCursorAccordingCoord(Point realPos) {
        ...
        realPos.y -= getY() + getPadding().top + globalYShift;
            ...
            switch (args.mods) {
                case SHIFT:
                    if (ShiftValCodes.contains(args.scancode)) {
                        //System.out.println(args.mods + " " + args.scancode + " " + _isSelect);
                        if (!_isSelect) {
                            _isSelect = true;
                            _selectFrom = new Point(_cursor_position);
                        }
    ...
    */
    ...
    public void pasteText(String pasteStr) {
        ...
        _cursor_position = checkLineFits(_cursor_position);
        String textBeg = new StringBuilder(_linesList.get(_cursor_position.y).getItemText()).substring(0, _cursor_position.x);
        String textEnd = "";
        if (_cursor_position.x < getLineLetCount(_cursor_position.y))
