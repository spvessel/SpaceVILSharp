...
class TextLine extends TextItem implements InterfaceTextContainer {
    ...
    */
    ...
    private void updateCoords() {
        ...
        List<com.spacevil.Flags.ItemAlignment> alignments = getTextAlignment();
        float alignShiftX = 1;
        float alignShiftY = 0;

        // Horizontal
        if (alignments.contains(com.spacevil.Flags.ItemAlignment.LEFT)) {
            alignShiftX = getParent().getPadding().left + getMargin().left;
        } else if (alignments.contains(com.spacevil.Flags.ItemAlignment.RIGHT) && (_lineWidth < _parentAllowWidth))
            alignShiftX = getParent().getWidth() - _lineWidth - getParent().getPadding().right - getMargin().right;

        else if (alignments.contains(com.spacevil.Flags.ItemAlignment.HCENTER) && (_lineWidth < _parentAllowWidth))
        ...
        // Vertical
        if (alignments.contains(com.spacevil.Flags.ItemAlignment.TOP)) {
            // System.out.println(getMargin().top);
            alignShiftY = getParent().getPadding().top + getMargin().top;
        } else if (alignments.contains(com.spacevil.Flags.ItemAlignment.BOTTOM))
            alignShiftY = getParent().getHeight() - height - getParent().getPadding().bottom - getMargin().bottom;

        else if (alignments.contains(com.spacevil.Flags.ItemAlignment.VCENTER))
    ...
    */
    ...
    float getLineTopCoord() {
        ...
        if (alignments.contains(com.spacevil.Flags.ItemAlignment.BOTTOM))
