...
import com.spacevil.Core.Cell;
import com.spacevil.Core.InterfaceGrid;
...
public class Grid extends Prototype implements InterfaceGrid {
    ...
    private Grid() {
        ...
        setStyle(com.spacevil.Common.DefaultsService.getDefaultStyle(Grid.class));
        isFocusable = false;
    }
    ...
    public void updateLayout() {
        ...
        for (int r = 0; r < _row_count; r++) {
            ...
            for (int c = 0; c < _column_count; c++) {
                ...
                if (item != null) {
                    if (item.getWidthPolicy() == com.spacevil.Flags.SizePolicy.EXPAND)
                        item.setWidth(columns_width[c] - item.getMargin().left - item.getMargin().right);

                    if (item.getHeightPolicy() == com.spacevil.Flags.SizePolicy.EXPAND)
    ...
    private int[] getRowsHeight() {
        ...
        for (int r = 0; r < _row_count; r++) {
            for (int c = 0; c < _column_count; c++) {
                ...
                if (item.getHeightPolicy() == com.spacevil.Flags.SizePolicy.FIXED) {
    ...
    private int[] getColumnsWidth() {
        ...
        for (int c = 0; c < _column_count; c++) {
            for (int r = 0; r < _row_count; r++) {
                ...
                if (item.getWidthPolicy() == com.spacevil.Flags.SizePolicy.FIXED) {
