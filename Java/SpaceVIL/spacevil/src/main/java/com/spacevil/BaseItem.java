...
public abstract class BaseItem implements InterfaceBaseItem {
    ...
    void addChildren(InterfaceBaseItem item) {
        ...
        if (item.getParent() instanceof InterfaceVLayout) {
            addEventListener(com.spacevil.Flags.GeometryEventType.RESIZE_WIDTH, item);
            addEventListener(com.spacevil.Flags.GeometryEventType.MOVED_X, item);
            castAndUpdate(item);
            return;
        }
        if (item.getParent() instanceof InterfaceHLayout) {
            addEventListener(com.spacevil.Flags.GeometryEventType.RESIZE_HEIGHT, item);
            addEventListener(com.spacevil.Flags.GeometryEventType.MOVED_Y, item);
            castAndUpdate(item);
            return;
        }
    ...
    public void removeItemFromListeners() {
        getParent().removeEventListener(com.spacevil.Flags.GeometryEventType.RESIZE_WIDTH, this);
        getParent().removeEventListener(com.spacevil.Flags.GeometryEventType.RESIZE_HEIGHT, this);
        getParent().removeEventListener(com.spacevil.Flags.GeometryEventType.MOVED_X, this);
        getParent().removeEventListener(com.spacevil.Flags.GeometryEventType.MOVED_Y, this);
    }
    ...
    void updateBehavior() {
        ...
        List<com.spacevil.Flags.ItemAlignment> alignment = getAlignment();
        ...
        if (alignment.contains(com.spacevil.Flags.ItemAlignment.HCENTER)) {
            setX(getParent().getX() + (getParent().getWidth() - getWidth()) / 2 + getMargin().left - getMargin().right);//
        }
        if (alignment.contains(com.spacevil.Flags.ItemAlignment.VCENTER)) {
    ...
    private void protoUpdateBehavior() {
        ...
        List<com.spacevil.Flags.ItemAlignment> alignment = prt.getAlignment();
     ...
     */
    public void setSizePolicy(com.spacevil.Flags.SizePolicy width, com.spacevil.Flags.SizePolicy height) {
     ...
     */
    public void update(com.spacevil.Flags.GeometryEventType type, int value) {
        if (this instanceof VisualItem) {
        ...
        case RESIZE_WIDTH:
            if (getWidthPolicy() == com.spacevil.Flags.SizePolicy.FIXED) {
                if (getAlignment().contains(com.spacevil.Flags.ItemAlignment.RIGHT)) {
                    setX(getParent().getX() + getParent().getWidth() - getWidth() - getParent().getPadding().right
                            - getMargin().right);//
                }
                if (getAlignment().contains(com.spacevil.Flags.ItemAlignment.HCENTER)) {
                    setX(getParent().getX() + (getParent().getWidth() - getWidth()) / 2 + getMargin().left
                            - getMargin().right);
                }
            } else if (getWidthPolicy() == com.spacevil.Flags.SizePolicy.EXPAND) {
        ...
        case RESIZE_HEIGHT:
            if (getHeightPolicy() == com.spacevil.Flags.SizePolicy.FIXED) {
                if (getAlignment().contains(com.spacevil.Flags.ItemAlignment.BOTTOM)) {
                    setY(getParent().getY() + getParent().getHeight() - getHeight() - getParent().getPadding().bottom
                            - getMargin().bottom);//
                }
                if (getAlignment().contains(com.spacevil.Flags.ItemAlignment.VCENTER)) {
                    setY(getParent().getY() + (getParent().getHeight() - getHeight()) / 2 + getMargin().top
                            - getMargin().bottom);
                }
            } else if (getHeightPolicy() == com.spacevil.Flags.SizePolicy.EXPAND) {
    ...
    private void protoUpdate(com.spacevil.Flags.GeometryEventType type, int value) {
        Prototype prt = ((VisualItem) this)._main;
        prt.setConfines();
        switch (type) {
        case MOVED_X:
        ...
        case RESIZE_WIDTH:
            if (prt.getWidthPolicy() == com.spacevil.Flags.SizePolicy.FIXED) {
                if (prt.getAlignment().contains(com.spacevil.Flags.ItemAlignment.RIGHT)) {
                    prt.setX(prt.getParent().getX() + prt.getParent().getWidth() - prt.getWidth()
                            - prt.getParent().getPadding().right - prt.getMargin().right);//
                }
                if (prt.getAlignment().contains(com.spacevil.Flags.ItemAlignment.HCENTER)) {
                    prt.setX(prt.getParent().getX() + (prt.getParent().getWidth() - prt.getWidth()) / 2
                            + prt.getMargin().left - prt.getMargin().right);
                }
            } else if (prt.getWidthPolicy() == com.spacevil.Flags.SizePolicy.EXPAND) {
        ...
        case RESIZE_HEIGHT:
            if (prt.getHeightPolicy() == com.spacevil.Flags.SizePolicy.FIXED) {
                if (prt.getAlignment().contains(com.spacevil.Flags.ItemAlignment.BOTTOM)) {
                    prt.setY(prt.getParent().getY() + prt.getParent().getHeight() - prt.getHeight()
                            - prt.getParent().getPadding().bottom - prt.getMargin().bottom);//
                }
                if (prt.getAlignment().contains(com.spacevil.Flags.ItemAlignment.VCENTER)) {
                    prt.setY(prt.getParent().getY() + (prt.getParent().getHeight() - prt.getHeight()) / 2
                            + prt.getMargin().top - prt.getMargin().bottom);
                }
            } else if (prt.getHeightPolicy() == com.spacevil.Flags.SizePolicy.EXPAND) {
    ...
    void updateGeometry() {
        update(com.spacevil.Flags.GeometryEventType.RESIZE_WIDTH, 0);
        update(com.spacevil.Flags.GeometryEventType.RESIZE_HEIGHT, 0);
        update(com.spacevil.Flags.GeometryEventType.MOVED_X, 0);
        update(com.spacevil.Flags.GeometryEventType.MOVED_Y, 0);
    }
    ...
    public com.spacevil.Flags.ItemRule HoverRule = com.spacevil.Flags.ItemRule.LAZY;
